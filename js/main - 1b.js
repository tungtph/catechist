  ...
  if (okay === true)
  {
    ...
    if ((IsBlank(pw)) || (pw === null)) {
      ...
      if (window.localStorage) {
        ...
        values.push(Enc(pw));
        values.push(expires);
        localStorage.setItem("sespw", values.join("||"));
      }  
    }
...
function GetCredential(frm) {
  ...
  if (window.localStorage) {
    ...
    if ((!IsBlank(sk)) && (sk !== null)) {
      ...
      values = [];
      checkval = [];
      checkval = localStorage.getItem("sespw");
      if (checkval !== null) {
        values = localStorage.getItem("sespw").split("||");
        if (values[1] < expires) {
          localStorage.removeItem("sespw");
        }
        else {
          pw1 = values[0];
          pw1 = Dec(pw1);
    ...
    if (window.localStorage) {
      ...
      if (!IsBlank(serverGUI)) {
        ...
        if (sx === "Yes")	{
          ...
          values.push(expires);
          var jv = values.join("||");
          localStorage.setItem("password", jv);
          localStorage.setItem("sespw", jv);                   
